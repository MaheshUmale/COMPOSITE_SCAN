...
for tf in timeframes:
    select_cols.extend([
        ...
        f'ADX{tf}', f'EMA20{tf}', f'SMA50{tf}'
    ])

def get_squeeze_breakout_query(settings):
...
def get_trending_pullback_query(settings):
    ...
    trend_filters = [And(col(f'ADX{tf}') > 25, col(f'close{tf}') > col(f'SMA50{tf}'), col(f'close{tf}').between(col(f'EMA20{tf}') * 0.98, col(f'EMA20{tf}') * 1.02)) for tf in HIGH_PROBABILITY_TFS]
...
def run_scan(settings, cookies, db_name):
    ...
    try:
        _, df_squeeze = get_squeeze_breakout_query(settings).get_scanner_data(cookies=cookies)
        _, df_pullback = get_trending_pullback_query(settings).get_scanner_data(cookies=cookies)

        if df_squeeze is not None and not df_squeeze.empty:
            df_squeeze['Strategy_Type'] = 'High_Potential'
        if df_pullback is not None and not df_pullback.empty:
            df_pullback['Strategy_Type'] = 'High_Probability'

        df = pd.concat([df_squeeze, df_pullback], ignore_index=True)
